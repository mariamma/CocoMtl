...
import utils
from coco_testutils import get_coco
...
def get_args_parser(add_help=True):
    ...
    parser.add_argument("--output-dir", default="/scratch/mariamma/coco/checkpoints/mtl/", type=str, help="path to save outputs")
    ...
    parser.add_argument(
        "--corruption", default=None, type=str, help="type of image corruption to be applied"
    )
    parser.add_argument("--severity", default=1, type=int, help="severity of image corruption")
...
def main(args):
    ...
    # model = maskrcnn_resnet50_fpn(weights=args.weights, weights_backbone=args.weights_backbone, num_classes=num_classes, **kwargs
    # )
    ...
    if args.test_only:
        print("Test only...")
        checkpoint = torch.load(os.path.join(args.output_dir, 'checkpoint.pth'))
        ...
        coco_evaluator = evaluate(model.module.maskrcnn, data_loader_test, args.corruption, args.severity, args.output_dir, device=device)
        # write_statsto_file(args.corruption, args.severity, coco_evaluator)
        return


def write_statsto_file(corruption, sev, coco_evaluator):
    filename = "res_" + corruption + ".txt"
